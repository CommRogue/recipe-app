...
- **H5, owner-private data is separable from the recipe body.** Rating, Collection membership and any snapshot of the Generation Request are not interleaved with ingredients and steps. The Generation Request matters most: it contains the user's Constraints, which can include allergies. If a Recipe stores the Constraints it was generated under (the validation ticket, #21, may want this), that snapshot must sit in a clearly separate field or document so a shared copy can leave it out.
...
- **H14, a post-processing stage in the Go service.** ADR 0002 names nutrition lookups as an extension of the service. The natural slot is after the model returns and before the Draft goes to the client, next to wherever a Draft is validated against Constraints (#21). v1 needs that stage to exist as a place, even if it does little.
